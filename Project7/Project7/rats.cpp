...
string Player::move(int dir)
{
	if (dir == NORTH) {
		...
		else if (m_arena->numberOfRatsAt(m_row - 1, m_col) > 0) {
			m_dead = true;
		...
		else if (m_arena->numberOfRatsAt(m_row, m_col + 1) > 0) {
			m_dead = true;
			return "Player walked into a rat and died.";
		}
		else {
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	...
	// Delete the following line and replace it with your code.
	 // This implementation compiles, but is incorrect.
	if (r - 1 > 0 && c - 1 > 0 && r + 1 <= a.rows() && c + 1 <= a.cols()) {
		if (a.numberOfRatsAt(r - 1, c) == 0 && a.numberOfRatsAt(r, c - 1) == 0 && a.numberOfRatsAt(r + 1, c) == 0
			&& a.numberOfRatsAt(r, c + 1) == 0) {
			return false;
		}
	}
	else if (c - 1 > 0 && r + 1 <= a.rows() && c + 1 <= a.cols()) {
		if (a.numberOfRatsAt(r, c - 1) == 0 && a.numberOfRatsAt(r + 1, c) == 0 && a.numberOfRatsAt(r, c + 1) == 0) {
			return false;
		}
	}
	else if (r - 1 > 0 && r + 1 <= a.rows() && c + 1 <= a.cols()) {
		if (a.numberOfRatsAt(r - 1, c) == 0 && a.numberOfRatsAt(r + 1, c) == 0 && a.numberOfRatsAt(r, c + 1) == 0) {
			return false;
		}
	}
	else if (r - 1 > 0 && c - 1 > 0 &&  c + 1 <= a.cols()) {
		if (a.numberOfRatsAt(r - 1, c) == 0 && a.numberOfRatsAt(r, c - 1) == 0 && a.numberOfRatsAt(r, c + 1) == 0) {
			return false;
		}
	}
	else if (r - 1 > 0 && c - 1 > 0 && r + 1 <= a.rows()) {
		if (a.numberOfRatsAt(r - 1, c) == 0 && a.numberOfRatsAt(r, c - 1) == 0 && a.numberOfRatsAt(r + 1, c) == 0) {
			return false;
		}
	}
	else if (r + 1 <= a.rows() && c + 1 <= a.cols()) {
		if ( a.numberOfRatsAt(r + 1, c) == 0 && a.numberOfRatsAt(r, c + 1) == 0) {
			return false;
		}
	}
	else if (r - 1 > 0 && c + 1 <= a.cols()) {
		if (a.numberOfRatsAt(r - 1, c) == 0 && a.numberOfRatsAt(r, c + 1) == 0) {
			return false;
		}
	}
	...
	else if (c - 1 > 0 && r + 1 <= a.rows() ) {
		if (a.numberOfRatsAt(r, c - 1) == 0 && a.numberOfRatsAt(r + 1, c) == 0) {
			return false;
		}
	}

	
		int dangerNorth = 0, dangerSouth = 0, dangerWest = 0, dangerEast = 0, dangerHere = 0;
		if (r - 1 > 0) {
			dangerHere += a.numberOfRatsAt(r - 1, c);
		}
		if (r + 1 <= a.rows()) {
			dangerHere += a.numberOfRatsAt(r + 1, c);
		}
		if (c - 1 > 0) {
			dangerHere += a.numberOfRatsAt(r, c - 1);
		}
		if (c + 1 > 0) {
			dangerHere += a.numberOfRatsAt(r, c + 1);
		}
		...
		if (r - 1 <= 0) {
			...
			if (r - 2 > 0) {
				dangerNorth += a.numberOfRatsAt(r - 2, c);
			}
			if (c + 1 <= a.cols()) {
				dangerNorth += a.numberOfRatsAt(r - 1, c + 1);
			}
			if (c - 1 > 0) {
				dangerNorth += a.numberOfRatsAt(r - 1, c - 1);
			}
		...
		if (r + 1 > a.rows()) {
			...
			if (r + 2 <= a.rows()) {
				dangerSouth += a.numberOfRatsAt(r + 2, c);
			}
			if (c + 1 <= a.cols()) {
				dangerSouth += a.numberOfRatsAt(r + 1, c + 1);
			}
			if (c - 1 > 0) {
				dangerSouth += a.numberOfRatsAt(r + 1, c - 1);
			}
		...
		if (c - 1 <= 0) {
			...
			if (c - 2 > 0) {
				dangerWest += a.numberOfRatsAt(r, c - 2);
			}
			if (r - 1 > 0) {
				dangerWest += a.numberOfRatsAt(r - 1, c - 1);
			}
			if (r + 1 <= a.rows()) {
				dangerWest += a.numberOfRatsAt(r + 1, c - 1);
			}
		...
		if (c + 1 > a.cols()) {
			...
			if (c + 2 <= a.cols()) {
				dangerEast += a.numberOfRatsAt(r, c + 2);
			}
			if (r - 1 > 0) {
				dangerEast += a.numberOfRatsAt(r - 1, c + 1);
			}
			if (r + 1 <= a.rows()) {
				dangerEast += a.numberOfRatsAt(r + 1, c + 1);
			}
		...
		if (dangerHere <= dangerNorth && dangerHere <= dangerSouth && dangerHere <= dangerWest && dangerHere <= dangerEast) {
			return false;
		}
		else if (dangerNorth <= dangerSouth && dangerNorth <= dangerWest && dangerNorth <= dangerEast) {
			bestDir = NORTH;
			cerr << bestDir << endl;
			return true;
		}
		else if (dangerSouth <= dangerEast && dangerSouth <= dangerWest) {
			bestDir = SOUTH;
			cerr << bestDir << endl;
			return true;
		}
		else if (dangerWest <= dangerEast) {
			bestDir = WEST;
			cerr << bestDir << endl;
			return true;
		}
